...
class Visual2(Visualization):
    ...
    def __init__(self, evening_file1, morning_file1, evening_file2, morning_file2, sensor_latitude, sensor_longitude, sensor_number):
    ...
    def _import_files(self):
        ...
        df_morn1 = pd.read_csv(file_path_morn1, low_memory=False, sep=";", names=col_names)
        df_eve1 = pd.read_csv(file_path_eve1, low_memory=False, sep=";", names=col_names)

        df_morn2 = pd.read_csv(file_path_morn2, low_memory=False, sep=";", names=col_names)
        df_eve2 = pd.read_csv(file_path_eve2, low_memory=False, sep=";", names=col_names)
    ...
    def create_visual(self, t_start1: datetime, t_end1: datetime, t_start2: datetime, t_end2: datetime):
        """
        ...
        trace2 = go.Scatter(x=x1, y=y2, mode='lines', name=f'{self.evening_day2} Light Frequency', yaxis='y1')

        # Create the first trace object with its own y-axis
        trace3 = go.Scatter(x=x1, y=y3, line=dict(dash='dash'), name=f'{self.evening_day1} Temp', yaxis='y2')

        # Create the second trace object with its own y-axis
        trace4 = go.Scatter(x=x1, y=y4, line=dict(dash='dash'), name=f'{self.evening_day2} Temp', yaxis='y2')

        # Define the layout object with two y-axes
        layout = go.Layout(title=f'{self.evening_day1} and {self.evening_day2} Sensor {self.sensor_number} Dark Sky Observation', xaxis=dict(title='Time (CST)'),
        ...
        fig.add_annotation(text=f'{self.evening_day1} Avg. Moon Illum: {df1["moon_illum"].mean():.1f}', x=60, y=0.9, showarrow=False)
        fig.add_annotation(text=f'{self.evening_day2} Avg. Moon Illum: {df2["moon_illum"].mean():.1f}', x=60, y=0.86, showarrow=False)

        cloudcover1 = self._call_weather_api(t_start1, t_end1)
        avg_cloud_cover1 = sum(cloudcover1.values()) / len(cloudcover1)
        fig.add_annotation(text=f'{self.evening_day1} Avg. Cloud Cover: {avg_cloud_cover1:.1f}%', x=60, y=0.82, showarrow=False)

        cloudcover2 = self._call_weather_api(t_start2, t_end2)
        avg_cloud_cover2 = sum(cloudcover2.values()) / len(cloudcover2)
        fig.add_annotation(text=f'{self.evening_day2} Avg. Cloud Cover: {avg_cloud_cover2:.1f}%', x=60, y=0.78, showarrow=False)
...
if __name__ == "__main__":
    vis1 = Visual2("2022-09-19_LENSSTSL0008.txt", "2022-09-20_LENSSTSL0008.txt", "2022-12-26_LENSSTSL0008.txt", "2022-12-27_LENSSTSL0008.txt", 42.57, -88.542, 8)
