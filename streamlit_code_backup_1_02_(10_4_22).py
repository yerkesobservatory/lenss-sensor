...
from PIL import Image
        ...
        #Page setup
st.set_page_config(page_title="LENSS Sensor Plotting", page_icon="🌠", initial_sidebar_state="expanded")
...
if input_password == "Y3rke5*": st.success("Correct Password!")
else: st.stop()
...
sensor_selection = st.selectbox(label = "Choose a Sensor", options = ('Sensor 5', 'Sensor 11', 'Sensor 12'))
st.caption("Listed sensors have all available data from 2022-06-01 to today")
day_selection = st.date_input("Choose a Night", datetime.date(datetime.now() - timedelta(2)))
...
night_error = False
if filetype == "New":
    try: sensor = pandas.read_csv("files/" + filename + ".txt", sep=';', names=["UTC", "CST", "Temperature", "Frequency", "Voltage", "Sensor Number"])
    except: morning_error = True
    try: sensor2 = pandas.read_csv("files/" + filename2 + ".txt", sep=';', names=["UTC", "CST", "Temperature", "Frequency", "Voltage", "Sensor Number"])
    except: night_error = True
else:
    try: sensor = pandas.read_csv("files/" + filename + ".txt", names=["Time", "Voltage", "Frequency", "Temperature", "Sensors"])
    except: morning_error = True
    try: sensor2 = pandas.read_csv("files/" + filename2 + ".txt", names=["Time", "Voltage", "Frequency", "Temperature", "Sensors"])
    except: night_error = True
...
if morning_error == True:
    ...
    if night_error == True: 
        ...
        st.stop()
if night_error == True: st.error("Missing File For The Night!")
if night_error == True or morning_error == True: st.stop()
...
while Frequency [last0] !=0: last0 = last0 -1
...
first0 = 240
while Frequency2 [first0] !=0: first0 = first0 +1  
...
morningmean = sum(Frequency_Morning [:len(Frequency_Morning)-60])/(len(Frequency_Morning)-60)
while sum(Frequency_Night[:5])/5 > morningmean + 2: Frequency_Night = Frequency_Night [5:] 
while sum(Frequency_Morning [len(Frequency_Morning) -5:])/5 > morningmean + 2: Frequency_Morning = Frequency_Morning [:len(Frequency_Morning) -5]
...
for word in list(sensor_options):
    if word in sensor_selection: sensor_options.remove(word)
...
with st.sidebar:
    ...
    overlay_toggle = st.radio("Overlay Other Sensor Data?", ('Deny Overlay', 'Allow Overlay'))
    if overlay_toggle == 'Allow Overlay': overlay_options = st.radio('Available Overlays', (sensor_options))
    ...
    with st.expander("Choose Colors", expanded = False):
        ...
        with option_column2:
            if goal_line: goal_color = st.color_picker('Goal Color', '#ff6600')
            if overlay_toggle == 'Allow Overlay': overlay_color = st.color_picker('Overlay Color', '#008000')
            if trendline: trendline_color = st.color_picker('Trendline Color', '#000000')

        #Allows user to select whether the temp should be read in Celsius or Fahrenheit
    c_or_f = st.radio("Read temperatures in Celsius of Fahrenheit?", ('Celsius', 'Fahrenheit'))
...
with st.expander("Advanced Options", expanded = False):    
    custom_x = st.slider("Select the X-Axis values of the graph:", value=(0,len(night)))
...
if timestamps: 
    ...
    plt.plot([len(Frequency_Night), len(Frequency_Night)], [-1, sum(Frequency_Night[len(Frequency_Night) -2:])/2], color=midnight_color, linestyle='solid', linewidth=2, label="Midnight")
    ...
    while PM_Plotting_Loop - 60 > list(custom_x)[0]:
        ...
        plottingloop_Y = [-1, sum(Frequency_Night[PM_Plotting_Loop:PM_Plotting_Loop +1])]
        plt.plot(plottingloop_X, plottingloop_Y, color=hours_color, linestyle='solid', linewidth=2)
    ...
    while AM_Plotting_Loop + 60 < list(custom_x)[1]:
        ...
        plottingloop_Y = [-1, sum(night[AM_Plotting_Loop:AM_Plotting_Loop +1])]
        plt.plot(plottingloop_X, plottingloop_Y, color=hours_color, linestyle='solid', linewidth=2)



        #When activated, adds a goal line to the graph by doing some totally complex math
if goal_line: plt.plot(range(len([x - (mean - 3) for x in night])), [x - (mean - 3) for x in night], color=goal_color, linestyle='solid', linewidth=2, label='Goal')


        #Sets Y-Axis values for the text in hour_labels to make them easy to view
if hour_labels:
    if custom_y <= 10: hour_labels_y = -0.5
    if custom_y < 30 and custom_y > 10: hour_labels_y = -1
    if custom_y >= 30 and custom_y <= 85: hour_labels_y = -3
    if custom_y > 85: hour_labels_y = -4


        #When toggled on, these loops add labeled hour markers to the bottom of the graph, can be buggy when the night is long
if hour_labels: 
    if list(custom_x)[0] < len(Frequency_Night) and list(custom_x)[1] > len(Frequency_Night): plt.text(len(Frequency_Night), hour_labels_y, '\ 12 AM')
...
if hour_labels:
    ...
    while PM_Label_Loop - 60 > list(custom_x)[0]:
        ...
        PM_Label_Loop = PM_Label_Loop - 60
        Midnight_PM = Midnight_PM - 1
        if PM_Label_Loop > 0 and PM_Label_Loop < list(custom_x)[1]: plt.text(PM_Label_Loop, hour_labels_y, '\ ' + str(Midnight_PM) + ' PM')
    ...
    while AM_Label_Loop + 60 < list(custom_x)[1]:
        ...
        AM_Label_Loop = AM_Label_Loop + 60
        Midnight_AM = Midnight_AM + 1
        if AM_Label_Loop > 0 and AM_Label_Loop < list(custom_x)[1]: plt.text(AM_Label_Loop, hour_labels_y, '\ ' + str(Midnight_AM) + ' AM')


        #Imports overlay files
if overlay_toggle == 'Allow Overlay':
    if overlay_options == 'Sensor 5': additional_sensor_number = "05"

    if overlay_options == 'Sensor 11': additional_sensor_number = "11"

    try: addition1night = pandas.read_csv("files/" + (str(day_selection) + "_LENSSTSL00" + additional_sensor_number) + ".txt", names=["Time", "Voltage", "Frequency", "Temperature", "Sensors"])
    except: night_error = True
    try: addition1morning = pandas.read_csv("files/" + (str(morning_selection) + "_LENSSTSL00" + additional_sensor_number) + ".txt", names=["Time", "Voltage", "Frequency", "Temperature", "Sensors"])
    except: morning_error = True        


    if overlay_options == 'Sensor 12':
        additional_sensor_number = "12"
        try: addition1night = pandas.read_csv("files/" + (str(day_selection) + "_LENSSTSL12") + ".txt", sep=';', names=["UTC", "CST", "Temperature", "Frequency", "Voltage", "Sensor Number"])
        except: night_error = True
        try: addition1morning = pandas.read_csv("files/" + (str(morning_selection) + "_LENSSTSL12") + ".txt", sep=';', names=["UTC", "CST", "Temperature", "Frequency", "Voltage", "Sensor Number"])
        except: morning_error = True
    ...
    if morning_error == True:
        ...
        if night_error == True: 
            ...
            st.stop()
    if night_error == True: st.error("Missing File For The Night Of Overlay!")
    if night_error == True or morning_error == True: st.stop()
    ...
    if overlay_options == 'Sensor 5' or 'Sensor 11' or 'Sensor 12':
        ...
        while Frequency_Night_Overlay [Additional_last0] !=0: Additional_last0 = Additional_last0 -1
        if Additional_last0 > len(Frequency_Night_Overlay [:len(Frequency_Night_Overlay) -120]):
            Additional_last0 = len(Frequency_Night_Overlay) -120
            while Frequency_Night_Overlay [Additional_last0] !=0: Additional_last0 = Additional_last0 -1

        Frequency_Night_Overlay = Frequency_Night_Overlay [Additional_last0 +1:]
        Additional_first0 = 240
        while Frequency_Morning_Overlay [Additional_first0] !=0: Additional_first0 = Additional_first0 +1  
        Frequency_Morning_Overlay = Frequency_Morning_Overlay [0:Additional_first0 -1]
        Additional_morningmean = sum(Frequency_Morning_Overlay)/len(Frequency_Morning_Overlay)
        while sum(Frequency_Night_Overlay[:5])/5 > Additional_morningmean + 2: Frequency_Night_Overlay = Frequency_Night_Overlay [5:]
        while sum(Frequency_Morning_Overlay [len(Frequency_Morning_Overlay) -5:])/5 > Additional_morningmean + 2: 
            Frequency_Morning_Overlay = Frequency_Morning_Overlay [:len(Frequency_Morning_Overlay) -5]
...
if overlay_toggle == 'Allow Overlay': plt.plot(range(len(Frequency_Night) - len(Frequency_Night_Overlay), len(additional_night) + len(Frequency_Night) - len(Frequency_Night_Overlay)), additional_night, linestyle='solid', linewidth=3, c=overlay_color, label=overlay_options)




        #Plots the main line for the night
arr = plt.plot(range(len(night)), night, linestyle='solid', linewidth=3, c=main_color, label=sensor_selection)
...
if overlay_toggle == 'Allow Overlay': plt.title('TSL 00' + sensornumber + " and TSL 00" + additional_sensor_number + " Night of " + filename[0:10])
if overlay_toggle == 'Deny Overlay': plt.title('TSL 00' + sensornumber + " Night of " + filename[0:10])

        #Plots the trendline
if trendline: plt.plot([0, len(night)], [sum(Frequency_Night[:20])/20, morningmean], color=trendline_color, linewidth=1.5)
...
if hour_labels: plt.xlabel("")
plt.ylabel("Frequency")
plt.xticks([])


        #Adds the mean to the top left of the graph
if meandisplay and custom_y > mean + 2: plt.text(0.1, 0.9, "Mean = " + str(round(mean, 2)), ha='center', va='center', transform=ax.transAxes)

        #Saves the file to allow its potential download, may need to be fixed as it could cause storage problems 
plt.savefig('Images/TSL ' + filename[19:] + " Night of " + filename[0:10] + '.png')
...
if custom_y < mean + 2 and meandisplay: st.warning("Unable to safely display the mean!")

if custom_y > mean + 2 and meandisplay: st.success('Graph Created Successfully!')

if meandisplay == False: st.success('Graph Created Successfully!')
...
temperature_column1.metric("Mean Temperature", str(round((temp_mean), 1)) + temp_unit)
temperature_column2.metric("Lowest Temperature", str(round((temp_min), 1)) + temp_unit, "-" + str(round((temp_mean - temp_min), 1)) + temp_unit)
temperature_column3.metric("Highest Temperature", str(round((temp_max), 1)) + temp_unit, str(round((temp_max - temp_mean), 1)) + temp_unit)


        #Creates a variable name with the file name (was saved earlier) in order for download by the viewer 
image_name = ('Images/TSL ' + filename[19:] + " Night of " + filename[0:10] + '.png')

        #Creates a download button to download the graph
with open(image_name, "rb") as file: st.download_button(label="Download Graph", data=file, file_name=image_name)


        #Allows the user to view the raw data for the night, defaults to collapsed as to not be annoying
with st.expander("View Raw Data"):
    st.subheader("Raw Data")
    combined_sensor = pandas.concat([sensor[last0+20:], sensor2[0:Frequency2.index(0) -50]], ignore_index=True, sort=False)
...
locations = pandas.DataFrame([
    ...
    (42.5734, -88.5739)], #S12 X 
    columns=['lat', 'lon'])
st.map(locations)

      #Description text
st.caption("All sensor locations are not precisely on their true location, and deviate by a noticable amount.")
