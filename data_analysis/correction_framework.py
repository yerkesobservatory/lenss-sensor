...
ensure desired time frame is present in the file
...
read each line, tokenize on semicolon, store in struct (maybe with additional info like hourly weather?)

starting in file2 with the first entry to the last entry before sunrise:
read each line, tokenize on semicolon, store in struct (maybe with additional info like hourly weather?)

output is a struct for the night, containing info like moon illumination and all relevant lines from the
files as their own struct within the night struct

functions to follow afterwards:
-writing a night struct to Google Drive
-take a night struct and update the values on some attribute
